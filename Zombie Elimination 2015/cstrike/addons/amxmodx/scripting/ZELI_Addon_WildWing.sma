...
public plugin_init()
{
	...
	register_clcmd("say /get1111", "Get_WildWing")
	
	g_Engineer = ZombieEli_GetClassID("Engineer")
	g_WildWing = ZombieEli_RegisterWeapon(g_Engineer, "CSO WildWing", WPN_SECONDARY, 0)
}
...
{
	if(ItemID == g_WildWing) Get_WildWing(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_WildWing) Remove_WildWing(id)
}
