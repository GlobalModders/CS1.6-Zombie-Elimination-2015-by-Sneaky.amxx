...
new g_Drill, g_Engineer
			   
public plugin_init()
{
	register_plugin(PLUGIN, VERSION, AUTHOR);
	
	register_clcmd("say /drill11", "clcmd_buydrill");
	...
	g_Drill = ZombieEli_RegisterWeapon(g_Engineer, "CSO Power Drill", WPN_PRIMARY, 5, 0)
}
...
{
	if(ItemID == g_Drill) clcmd_buydrill(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_Drill) DropAllPrimary(id)
}
