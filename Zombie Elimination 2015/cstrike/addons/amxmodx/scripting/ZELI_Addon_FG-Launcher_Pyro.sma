...
public plugin_init() 
{
	...
	register_clcmd("say /ge131t", "Get_FGLauncher")

	g_PyroTech = ZombieEli_GetClassID("Pyro Tech")
	g_FG = ZombieEli_RegisterWeapon(g_PyroTech, "CSO FG-Launcher", WPN_PRIMARY, 5, 0)
}
...
{
	if(ItemID == g_FG) Get_FGLauncher(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_FG) Remove_FGLauncher(id)
}
