...
new g_SunHongKong, g_Stick

native SunHK_Get_SunReachSP(id)
...
public plugin_init()
{
	...
	register_clcmd("say /get31312", "Get_RuyiStick")
	g_SunHongKong = ZombieEli_GetClassID("Sun Wukong")
	g_Stick = ZombieEli_RegisterWeapon(g_SunHongKong, "CSO Wukong Ruyi Stick", WPN_MELEE, 0, 0)
}
...
{
	if(ItemID == g_Stick) Get_RuyiStick(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_Stick) Remove_RuyiStick(id)
}

public zeli_class_unactivate(id, ItemID)
{
	if(ItemID == g_SunHongKong) Remove_RuyiStick(id)
}

public Remove_RuyiStick(id)
{
	UnSet_BitVar(g_Had_RuyiStick, id)
}
...
public Do_StabNow(id)
{
	...
	static SP; SP = SunHK_Get_SunReachSP(id)
	...
	switch(SP)
	{
		...
		case 2: Increase = 2.0
		case 3: Increase = 3.0
		default: Increase = 1.0
	}
	
	Point_Dis = 60.0
	Max_Distance = DISTANCE * Increase
	...
	for(new i = 0; i < get_maxplayers(); i++)
	{
		...
		|| get_distance_f(VicOrigin, Point[3]) <= Point_Dis)
		{
			...
			if(ZombieEli_GetLevel(id, ZombieEli_GetClass(id)) >= 10) do_attack(id, i, ent, DAMAGE_B * 3.0)
			else do_attack(id, i, ent, DAMAGE_B)
		}
	}	
...
public fw_TraceLine(Float:vector_start[3], Float:vector_end[3], ignored_monster, id, handle)
{
	...
	xs_vec_mul_scalar(v_forward, DISTANCE * Increase, v_forward)
...
public fw_PlayerTakeDamage(Victim, Inflictor, Attacker, Float:Damage, DamageBits)
{
	...
	{
		if(ZombieEli_GetLevel(Attacker, ZombieEli_GetClass(Attacker)) >= 10) SetHamParamFloat(3, DAMAGE_A * 3.0)
		else SetHamParamFloat(3, DAMAGE_A)
	}
}
