...
public plugin_init()
{
	...
	register_clcmd("say /get2", "Get_M79G")
	
	g_Engineer = ZombieEli_GetClassID("Engineer")
	g_M79GOLD = ZombieEli_RegisterWeapon(g_Engineer, "CSO M719GOLD", WPN_SECONDARY, 0)	
}
...
{
	if(ItemID == g_M79GOLD) Get_M79G(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_M79GOLD) Remove_M79G(id)
}
