...
public plugin_init()
{
	...
	register_clcmd("say /get11", "Get_Kujang")
	g_PyroTech = ZombieEli_GetClassID("Pyro Tech")
	g_horn = ZombieEli_RegisterWeapon(g_PyroTech, "CSO Horn Kujang", WPN_MELEE, 0, 0)
}
...
{
	if(ItemID == g_horn) Get_Kujang(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_horn) Remove_Kujang(id)
}
