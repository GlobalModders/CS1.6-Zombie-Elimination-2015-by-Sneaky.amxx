...
#define PLUGIN "[ZELI] Addon: Salamander"
#define VERSION "2015"
...
public plugin_init()
{
	...
	g_Salamander = ZombieEli_RegisterWeapon(g_PyroTech, "Salamander", WPN_PRIMARY, 0, 0)
...
{
	if(ItemID == g_Salamander) Remove_Salamander(id)
}
		...
		else if(is_alive(id)) 
		{
			...
			if(get_user_team(id) != EntTeam)
			{
				...
				if(is_connected(Attacker))
				{
					...
					if(is_alive(id)) PyroTech_AfterBurn(id, Attacker)
				}
			}
		}
	}
