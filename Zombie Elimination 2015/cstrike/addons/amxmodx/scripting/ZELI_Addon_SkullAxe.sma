...
#define PLUGIN "[ZD] Melee Weapon: Skull-Axe"
#define VERSION "1.0"
#define AUTHOR "Dias Leon"

#define V_MODEL "models/ZombieDarkness/Weapon/Melee/v_skullaxe.mdl"
#define P_MODEL "models/ZombieDarkness/Weapon/Melee/p_skullaxe.mdl"
...
#define SLASH_ROTATE_DAMAGE 800.0
#define SLASH_ROTATE_RADIUS 100.0
#define SLASH_ROTATE_POINT_DIS 60.0
#define SLASH_ROTATE_DELAY_TIME 1.1
#define SLASH_ROTATE_RESET_TIME 1.0

#define STAB_DAMAGE 800.0
#define STAB_RADIUS 90.0
#define STAB_POINT_DIS 90.0
#define STAB_TIME 1.0
#define STAB_RESET_TIME 0.5

#define TASK_SLASHING 2034+21
#define TASK_STABING 2034+11
...
new const SkullAxe_Sound[6][64] = 
{
	"weapons/skullaxe_draw.wav",
	"weapons/skullaxe_slash_hit.wav",
	"weapons/skullaxe_slash_miss.wav",
...
new g_DragonSword
new g_Had_DragonSword[33], g_Attack_Mode[33], g_Checking_Mode[33], g_Hit_Ing[33]
new g_Old_Weapon[33], g_Ham_Bot

new g_Heavy, g_SkullAxe
...
public plugin_init()
{
	...
	//g_DragonSword = zd_weapon_register("Skull-Axe", WEAPON_MELEE, CSW_KNIFE, 6000)
	
	g_Heavy = ZombieEli_GetClassID("Heavy")
	g_SkullAxe = ZombieEli_RegisterWeapon(g_Heavy, "CSO Skull Axe", WPN_MELEE, 0, 0)	
}
...
public plugin_precache()
{
	...
	for(new i = 0; i < sizeof(SkullAxe_Sound); i++)
		engfunc(EngFunc_PrecacheSound, SkullAxe_Sound[i])
}

public zeli_weapon_selected(id, ItemID, ClassID)
{
	if(ItemID == g_SkullAxe) get_dragonsword(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_SkullAxe) remove_dragonsword(id)
}
...
public get_dragonsword(id)
{
	...
	if(get_user_weapon(id) == CSW_KNIFE)
	{
		...
		set_weapon_anim(id, S9_ANIM_DRAW)
...
public Event_CurWeapon(id)
{
	...
	if(get_user_weapon(id) == CSW_DRAGONSWORD && g_Had_DragonSword[id] && g_Old_Weapon[id] != CSW_DRAGONSWORD)
	{
		set_weapon_anim(id, S9_ANIM_DRAW)
		
		set_player_nextattack(id, DRAW_TIME)
		set_weapons_timeidle(id, CSW_DRAGONSWORD, DRAW_TIME + 0.5)
	}
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if (CurButton & IN_ATTACK)
	{
		...
		set_weapon_anim(id, S9_ANIM_SLASH_BEGIN)
		...
		set_player_nextattack(id, SLASH_ROTATE_RESET_TIME + 0.5)
		set_weapons_timeidle(id, CSW_DRAGONSWORD, SLASH_ROTATE_RESET_TIME + 1.0)
		
		set_weapon_anim(id, S9_ANIM_STAB)
...
public Do_Slashing_Rotate(id)
{
	...
	{
		emit_sound(id, CHAN_WEAPON, SkullAxe_Sound[3], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, SkullAxe_Sound[4], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public Do_StabNow(id)
{
	...
	{
		set_weapon_anim(id, S9_ANIM_SLASH_HIT)
		emit_sound(id, CHAN_WEAPON, SkullAxe_Sound[3], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) 
		{
			set_weapon_anim(id, S9_ANIM_SLASH_HIT)
			emit_sound(id, CHAN_WEAPON, SkullAxe_Sound[4], 1.0, ATTN_NORM, 0, PITCH_NORM)
		} else if(g_Hit_Ing[id] == HIT_NOTHING) {
			set_weapon_anim(id, S9_ANIM_SLASH_MISS)
		}
	}
