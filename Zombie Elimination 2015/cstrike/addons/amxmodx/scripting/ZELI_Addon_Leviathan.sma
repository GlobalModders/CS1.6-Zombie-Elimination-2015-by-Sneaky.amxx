...
public plugin_init()
{
	...
	register_clcmd("say /get112", "Get_Salamander")
	register_clcmd("weapon_watercannon", "Hook_Weapon")

	g_FrozenTech = ZombieEli_GetClassID("Frozen Tech")
	g_Leviathan = ZombieEli_RegisterWeapon(g_FrozenTech, "CSO Leviathan", WPN_PRIMARY, 0, 0)
...
{
	if(ItemID == g_Leviathan) Get_Salamander(id)
}

public zeli_weapon_removed(id, ItemID)
{
	if(ItemID == g_Leviathan) Remove_Salamander(id)
}
