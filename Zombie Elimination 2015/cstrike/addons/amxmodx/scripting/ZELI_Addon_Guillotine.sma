...
#define MAX_RADIUS 640.0
#define GUILLOTINE_HITTIME 5.0

#define MODEL_V "models/ze/v_guillotine.mdl"
#define MODEL_P "models/ze/p_guillotine.mdl"
//#define MODEL_W "models/ze/w_guillotine.mdl"

#define MODEL_S "models/ze/guillotine_projectile.mdl"
#define MODEL_GIB "models/ze/gibs_guilotine.mdl"
...
new g_Had_Guillotine, g_InTempingAttack, g_CanShoot, g_Hit, g_Ammo[33], g_MyGuillotine[33], Float:g_DamageTime[33]
...
public plugin_init()
{
	...
	register_clcmd("say /guillotine11", "Get_Guillotine")
	register_clcmd("weapon_guillotine", "HookWeapon")
	
	g_Wukong = ZombieEli_GetClassID("Sun Wukong")
	g_Guillotine = ZombieEli_RegisterWeapon(g_Wukong, "CSO Guillotine 血滴子", WPN_PRIMARY, 0, 1)
}
	...
	{ // Touch Human
		...
		if(get_distance_f(EntOrigin, HeadOrigin) <= 16.0)
		{
			if(!pev(Ent, pev_return))
			{
				...
				if(get_gametime() - 0.25 > g_DamageTime[id])
				{	
					ExecuteHamB(Ham_TakeDamage, Touched, 0, id, float(DAMAGE) / 2.5, DMG_SLASH)
					g_DamageTime[id] = get_gametime()
				}
			}
		} else {
			if(get_gametime() - 0.25 > g_DamageTime[id])
		...
		if(!pev(Ent, pev_return))
		{
			...
			if(!Get_BitVar(g_Hit, id)) Guillotine_Broken(Ent, id)
			return
		}
	}
}
	...
	if(pev(Ent, pev_return)) // Returning to the owner
	{
		...
		if(!is_alive(Target))
		{
			...
			if(entity_range(Ent, id) > 100.0)
			{
				...
				Hook_The_Fucking_Ent(Ent, Origin, 1000.0)
			} else {
			...
			if(get_gametime() >= fTimeRemove)
			{
				...
				engfunc(EngFunc_SetOrigin, Ent, HeadOrigin)
				
				if(get_gametime() - 0.1 > g_DamageTime[id])
				{	
					// Animation
					set_pev(Ent, pev_animtime, get_gametime())
					set_pev(Ent, pev_framerate, 2.0)
					...
					do_attack(id, Target, fm_get_user_weapon_entity(id, CSW_KNIFE), float(DAMAGE) / 2.0)
...
public Guillotine_Broken(Ent, id)
{
...
public Create_Guillotine(id)
{
	...
	set_pev(iEnt, pev_mins, Float:{-16.0, -16.0, -2.0})
	set_pev(iEnt, pev_maxs, Float:{16.0, 16.0, 2.0})
